...
| Windows PowerShell 5.1·PowerShell 7 | Bootstrap, 동기화, Preflight, 검증 | Windows는 5.1 또는 7, macOS·Linux는 7로 같은 Workspace 절차를 사용한다. |
...
| 기술 | 기준 버전 | 사용 목적과 근거 |
|---|---:|---|
| eGovFrame Boot / Spring Boot | Parent 5.0.0 / Parent에서 상속 | 공공 프로젝트 친화 기반 위에서 API와 운영 기능을 일관되게 제공한다. |
...
| Spring AI | Product 1.1.8 / Control 1.0.1 | AX Module Studio의 승인된 Model Gateway 기반이다. 수동 CMS에서는 신규 AI 기능을 만들지 않는다. |
...
| 기술 | 기준 | 사용 목적과 근거 |
|---|---|---|
| Python·uv | `pyproject.toml` | 하나의 MCP Service와 `common`, `coding`, `cms` Package를 재현 가능하게 실행한다. |
| FastMCP | Source Manifest | 고정 Catalog의 Coding 7개·CMS 6개 Tool을 한 Endpoint로 제공한다. |

## 로컬 통합 실행

| 기술 | 기준 버전 | 사용 목적과 근거 |
|---|---:|---|
| Docker Compose | Compose Spec | DB·Migration·Backend·Frontend·Nginx·Orchestrator·MCP Server를 같은 로컬 절차로 실행한다. |
