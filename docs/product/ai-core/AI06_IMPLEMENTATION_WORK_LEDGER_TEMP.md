...
- Orchestrator `dev`는 현행 Coding Graph·Worker production 경로를 호환 Adapter로 유지하면서 주입형 Snapshot Provider/Runner와 Spring Profile Version 읽기 Client/Provider 경로를 추가했고, 107개 회귀 테스트가 있다. 실제 Job별 Profile 선택·production 연결은 아직 하지 않았다.
- 기존 Runtime을 새로 쓰거나 한 번에 교체하지 않는다. Snapshot 경로를 옆에 추가하고 기존 경로를 회귀 기준으로 유지한 뒤 단계적으로 연결한다.
- Backend 현재 기본 Checkout에는 별도 인증/JWT 작업의 미커밋 변경이 많다. 구현 시 이 Checkout은 건드리지 않고 최신 `origin/dev` 기반 독립 Worktree를 사용한다.
