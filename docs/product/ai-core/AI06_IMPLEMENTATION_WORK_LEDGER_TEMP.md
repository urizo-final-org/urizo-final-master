...
| 13 | `AI06-018` / `axms-ai06-018-provider-native-tool-calling` | Backend | 공통 Tool 정의·JSON Schema 검증과 OpenAI·Gemini Native Tool Calling 요청/응답 변환을 기존 Provider Gateway 경계에 연결한다. | Tool allowlist·Schema 제한·결정적 Tool Call 결과 계약·오류 거부·Secret/원문 응답 비노출 집중 회귀 통과 | `AI06-019`의 Stage DTO/Client·`CodingModelTurnService`·`modelBinding` 계약은 수정하지 않는다. Anthropic 유료 호출과 Frontend는 제외한다. | `Backend dev 병합 완료 · PR #37 / 9189d27 · Commit af24ce2 · AI06-019 파일 교집합 0` |
...
- `AI06-018` Backend Commit `af24ce2ddcfcee8c0d219259a68ec5d74af88c57` / PR [#37](https://github.com/urizo-final-org/urizo-final-backend/pull/37)은 Merge Commit `9189d27510e16c3c11c350205e6b53c14ebe2f2a`로 `dev` 병합 완료했다. 집중 테스트 45개와 실제 Spring AI OpenAI `tools.function`·Gemini `functionDeclarations` 직렬화 및 Gemini `thoughtSignature` 후속 요청 검증을 통과했다. Product 271개·Control 261개 전체 회귀는 기존 Windows CRLF 경계 테스트 각 1개만 실패하고 환경 Gate 각 4개를 Skip했으며, 두 Lane Package는 통과했다. Anthropic 유료 성공 호출과 외부 Provider 네트워크 왕복은 실행하지 않았다.
...
- 구현 Checkpoint:
  ...
  - [x] 기존 `app.ai_profile_version` Table·상태·불변 Trigger를 유지하고 `ai_workspace`에 INSERT·UPDATE만 추가
  - [x] Frontend Agent 설정 조회·저장·활성화, 중앙 활성 Guardrail 읽기 전용 표시, 잠금 Guardrail 삭제·비활성화 차단
- 검증 결과:
  - Frontend 고정 Node 24.14.0/pnpm 11.9.0 Image에서 Vitest 49개, TypeScript app/node 검사와 Vite production bundle 통과
