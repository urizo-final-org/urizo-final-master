...
- [x] RAG_QUALITY_BUILD 또는 RAG_REBUILD Job Type이 실제로 필요한 실행 시간·복구 조건
- [x] 재평가와 재빌드를 같은 Job으로 묶을지 분리할지 — 방향만 확정, 세부는 Scheduler 작업에서
- [ ] 실패·재시도와 Knowledge Version 활성화 차단 기준 — 절반 확정, 임계값 정책은 팀 논의 필요
- [x] 기존 Product Queue·Outbox 복구 계약 재사용 여부

### 2026-08-30 담당자 확인 결과

> 확인자: 민은지 (`emilyjjang-jpg`) · 실행 시간 근거는 로컬 bge-m3 CPU 실측값

- **Job Type 필요 여부: 필요하다고 판단.** 임베딩이 문서당 약 837ms로, 500건 기준 약 7분이 걸린다.
  동기 API로 처리할 수 없는 시간이므로 기존 Product Queue에 Job Type을 추가하는 방향에 동의한다.
  다만 이 항목이 6번 Agent 설정 스키마와도 연결되므로 **6번 담당자의 교차 확인을 요청한다.**
- **재평가·재빌드 분리: 분리 제안.** 재빌드는 임베딩을 포함해 7분대, 재평가는 검색만 수행해
  약 20초(쿼리 76ms/건)로 비용이 약 20배 차이 난다. 임베딩이 바뀌지 않은 경우까지 재빌드를
  돌리는 것은 낭비다. **오늘은 분리 방향만 확정하고**, 세부 설계는 9/2 이후 Scheduler 자동화
  작업에서 다시 논의한다.
- **활성화 차단 기준: 두 가지로 나눠 답한다.**
  - **빈 버전 차단은 반영한다.** 현재 활성화가 임베딩·청크 유무를 검사하지 않아 빈 버전도
    조용히 `ACTIVE`가 되고 검색이 `REFUSED`로 빠진다. 이는 버그로 보고 최소 방어
    (임베딩 건수 0이면 활성화 차단)를 넣는다.
  - **품질 지표 미달 시 자동 차단은 보류한다.** 임계값 설정은 정책 결정이라 담당자 단독으로
    정하지 않는다. 현재 기능 정의서도 "대시보드 알림"까지만 규정하므로, 자동 차단까지 갈지는
    팀 논의 후 결정한다.
- **Queue·Outbox 재사용: 동의.** Job Type 추가 여부와 별개로 기존 복구 계약을 재사용한다.

### 6번 담당자 회신 요청

- RAG 품질 Job Type 추가가 6번 Agent 설정 스키마에 영향을 주는지 확인 요청 (위 첫 항목).
