...
- [x] Natural CMS Job Outbox 멱등 INSERT에 필요한 `event_key` 최소 조회 권한 추가
- [x] Flyway 회귀 테스트와 PostgreSQL 권한·트랜잭션 검증
- [ ] 실제 MENU Job이 `WAITING_APPROVAL`까지 도달하고 승인 전 CMS 원본을 변경하지 않는지 확인
- 통합 결과: Outbox 권한 오류는 해소되어 MENU Job 생성과 `cms.analyze`까지 통과했다. 기존 `cms.preview` 프롬프트가 CONTENT 필드를 고정해 `CMS_COMMAND_INVALID`로 중단되며, Resource별 명령 프롬프트 보완은 다음 Work ID로 분리한다. MENU 원본은 변경되지 않았다.
...
- [x] 실제 MENU Job이 `WAITING_APPROVAL`까지 도달하고 승인 전 원본을 변경하지 않는지 검증
- 작업자: `tmdwns0531` (팀장 승인 재배정)
- 검증: 다중 Model·Tool 반복과 자유형 terminal JSON 파싱을 제거했으며 Natural CMS Stage·Resource·Model 경계·Migration 테스트 43개가 통과했다.
- 통합 결과: Backend 격리 재빌드와 `full` Health 통과 후 실제 MENU Job `269e8600-07ff-4d8b-86be-c4f28ee49f2a`이 `cms.analyze → cms.preview`를 거쳐 `WAITING_APPROVAL`에 도달했다. Preview가 생성됐고 비-MENU Handler Result는 0건이며, 승인 전 MENU 원본 `소개1`·`/about`은 변경되지 않았다.

| Work ID | Work slug | 작업 요약 | 저장소 | 진행 상태 | Branch | 최근 Push SHA·일자 | PR·상태·생성일 | dev 병합 SHA·일자 |
|---|---|---|---|---|---|---|---|---|---|
| AI05-013 | axms-ai05-013-natural-cms-command-schema | 5번 기능 작업 기록 | Master | PR 검증 중 | `feature/tmdwns0531_axms-ai05-013-natural-cms-command-schema_v0.1` | `93c42fa` · 2026-09-03 | [Master #57](https://github.com/urizo-final-org/urizo-final-master/pull/57) · OPEN · 2026-09-03 | dev 병합 전 |
| AI05-013 | axms-ai05-013-natural-cms-command-schema | Natural CMS 명령 구조 강제 | Backend | PR 검증 중 | `feature/tmdwns0531_axms-ai05-013-natural-cms-command-schema_v0.1` | `4d09ca1` · 2026-09-03 | [Backend #52](https://github.com/urizo-final-org/urizo-final-backend/pull/52) · OPEN · 2026-09-03 | dev 병합 전 |
