...
5. 먼저 읽기 전용 Preflight를 수행하고 Git, branch/HEAD/dirty,
   Docker/WSL, DB, Port, Health 상태를 보고하라.
6. Routine PowerShell, Git, Docker, Maven, Node, Python 명령은 네가
   version-managed script로 실행하라. 내가 수동으로 복사하게 하지 마라.
7. Network, Git/Docker 로그인, MFA, 관리자 권한, 설치, 재부팅,
   Secret 등록, local-full 변경 경계는 정확히 설명하고 승인을 요청하라.
8. 승인 후 Master bootstrap wrapper로 누락된 Source dev branch를
   sibling에 구성하고 Backend local-full bootstrap을 위임 실행하라.
9. 기존 비어 있지 않은 폴더, 다른 origin, dirty worktree를 덮어쓰지 마라.
   reset, clean, stash, checkout, rebase, DB 초기화, Volume 삭제를 금지한다.
10. Secret 원문이나 전체 digest를 채팅, 명령, 로그에 출력하지 마라.
11. 마지막에 workspace health를 실행하고 다음 형식으로 보고하라.
...
12. 세팅 중 제품 구현, commit, push, PR은 하지 마라.
13. 완료 후 AX-Module-Studio.code-workspace를 개발 진입점으로 안내하라.
```
