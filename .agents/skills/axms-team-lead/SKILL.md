...
12. Monitor dispatched sessions with the host's wait or status mechanism until each completes, blocks, or needs user input. At every state change, inspect the assigned scope and Git Diff; if `SIMPLE PASS` or `GUARDRAIL PASS` is violated, stop the affected session immediately and provide only the minimum correction. Run the independent verification session after its dependencies complete and finish with `TEAM MONITOR PASS`.
